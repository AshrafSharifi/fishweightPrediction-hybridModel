...
for _,time_row in df_timeWindow.iterrows():
    ...
    # df_temperature_data['df_temperature_function'] = df_temperature_data['PREORE_FEM_ENTRANCE-Temp [Â°C]'].apply(rainbow_trout_model.Temperature_function)
    ...
    # if time_row.sampling_rate_per_day==25:
    #     df_temperature_data['date_hour'] = df_temperature_data['Entrance_timestamp'].dt.strftime('%Y-%m-%d %H')
    #     VAKI_Size_Weigth['date_hour'] = VAKI_Size_Weigth['observed_timestamp'].dt.strftime('%Y-%m-%d %H')
    
    #     merged_df = pd.merge(
    #         df_temperature_data,
    #         VAKI_Size_Weigth[['date_hour', 'PREORE_VAKI-Weight [g]', 'observed_timestamp']],
    #         on='date_hour',
    #         how='left'
    #     )
    ...
    # Merge with a tolerance of 5 minutes (300 seconds)
