...
def get_split_indices(args):
    ...
    for size in args.time_windows_size:
        ...
        current_partitions = []
        current_start = 0  # Reset start index to 0 for each list
        ...
        # Store the partitions for the current time window
    ...
    for i in range(args.num_splits):
        ...
        for fold, (train_index, val_index) in enumerate(kfold.split(x_all)):
            ...
            if fold + 1==args.n_splits:
                title = "Time window: " +str(args.selected_tw)+ args.time_windows[args.selected_tw] + "  (Train size: "+str(len(x_train_chunk))+")"
                plots = Custom_plots(predicted_values,actual_values,writer=writer,title=title,summarytitle=args.run_name+f"_Incremental Training Step {i+1} (TimeWindow:{args.selected_tw})")
                plots.plot_all()
                plt.close(fig)
                print("*****************************************************************************************")  
        ...
        total_metrics[str(i)] = {"train_size": int(len(x_train_chunk)),"val_size": int(len(x_val_chunk)), "mse": mse1, "mae": mae1, "mape": mape1}
