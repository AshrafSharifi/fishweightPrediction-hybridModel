...
for _,time_row in df_timeWindow.iterrows():
    ...
    mean_random_weights = sum(initial_weights)/num_samples
    # =======================================================    
    ...
    for index,row in df_temperature_data.iterrows():
        ...
        df_temperature_data.at[index,'Dynamic_individual_weight'] = rainbow_trout_model.Dynamic_individual_weight(row['Energy_Acquisition(A)'],row['Catabolic_component(C)'],row['Somatic_tissue_energy_content(Epsilon)'])
