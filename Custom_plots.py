...
class Custom_plots:
    ...
    def plot_actual_vs_predicted_scatter(self):
        ...
        plt.plot([min(self.actual_values), max(self.actual_values)],
                 [min(self.actual_values), max(self.actual_values)],
                 color="red", linestyle="--", linewidth=2, label="Perfect Fit")
        plt.xlabel("Predicted Fish weights", fontsize=14)
        plt.ylabel("Actual Fish weights", fontsize=14)
        plt.title("Scatter plot: Actual vs Predicted Values", fontsize=14, fontweight="bold")
        plt.legend()
        plt.show()
    ...
    def plot_predictions_with_hist(self):
        bins = np.histogram_bin_edges(np.concatenate([self.actual_values, self.predicted_values]), bins=30)
        plt.figure(figsize=(8, 6))
        plt.hist(self.actual_values, bins=bins, label="Actual Values", alpha=0.6, color="blue", edgecolor="black")
        plt.hist(self.predicted_values, bins=bins, label="Predicted Values", alpha=0.6, color="orange", edgecolor="black")
        plt.title("Distribution of Predicted vs Actual Values", fontsize=14, fontweight="bold")
        plt.xlabel("Fish Weights", fontsize=14)
