...
class Args:
    ...
    verbose: int = 0
    num_splits: int = 3     # For incremental learning (optional)
    # Preprocessing
...
def incremental_train(args, df_hourly, df_daily):
    ...
    total = len(X_all)
    block = total // (args.num_splits + 1)

    fold_metrics = {}
    incremental_model = None

    # Fit scaler on the FIRST block only
    args.scaler_x.fit(X_all[:block].reshape(-1, X_all.shape[-1]))

    for step in range(1, args.num_splits + 1):
        print(f"\n=== Incremental Step {step} ===")

        # EXPANDING TRAINING SET
        train_end = step * block
        val_start = train_end
        ...
        # Scale
        X_train_scaled = args.scaler_x.transform(
            X_train.reshape(-1, X_train.shape[-1])
        ).reshape(X_train.shape)

        X_val_scaled = args.scaler_x.transform(
        ...
        # Predict
        y_pred = incremental_model.predict(X_val_scaled).flatten()

        # Reconstruct weight
        initial_log_weight = df_daily["log_weight"].iloc[val_start + 1]
        log_weight_pred = initial_log_weight + np.cumsum(y_pred)
        weight_pred = np.exp(log_weight_pred)

        weight_true = df_daily["smooth_weight"].iloc[val_start+1 : val_start+1+len(weight_pred)].values
...
if __name__ == "__main__":
    ...
    fold_metrics = incremental_train(args, df_hourly, df_daily)
