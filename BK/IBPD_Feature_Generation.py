...
for i in range(len(data)):
    ...
    rainbow_trout = rainbow_trout_model(sampling_rate_per_day)
    ...
    VAKI_Size_Weigth['PREORE_VAKI-Weight [g]'] = general.interpolate_outliers(VAKI_Size_Weigth['PREORE_VAKI-Weight [g]'])
    ...
    for index,row in merged_df.iterrows():
        ...
        merged_df.at[index,'Energy_Acquisition(A)'] = rainbow_trout.Energy_Acquisition(fish_weight,water_temperature)   
        merged_df.at[index,'Catabolic_component(C)'] = rainbow_trout.Catabolic_component(fish_weight,water_temperature)
        merged_df.at[index,'Somatic_tissue_energy_content(Epsilon)'] = rainbow_trout.Total_energy_input(fish_weight)
        merged_df.at[index,'Feeding_Amount'] = rainbow_trout.Input_ration(fish_weight,water_temperature)
        row = merged_df.loc[index]
        delta_t = 24/(rainbow_trout.constants_obj.delta_t*sampling_rate_per_day)
        merged_df.at[index,'Dynamic_individual_weight'] = rainbow_trout.Dynamic_individual_weight(row['Energy_Acquisition(A)'],row['Catabolic_component(C)'],row['Somatic_tissue_energy_content(Epsilon)'],delta_t)
